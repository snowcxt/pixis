...
module Home {
    ...
    export function init() {
        ...
        var stage = new Pixis.Stage(600, 400, <HTMLCanvasElement>document.getElementById("stage"), { webgl: false });
        var back = new Pixis.Image({
            x: 0,
            y: 0,
            image: new Pixis.ImageTexture("demo/home-showcase.jpg")
        });
        stage.add(back);
    ...
    function explode(x, y, size, color, explosion: Pixis.Group) {
    ...
    class Shape {
        ...
        shape1: Pixis.RegularPolygon;
        shape2: Pixis.RegularPolygon;
        stage: Pixis.Stage;
        explosion: Pixis.Group;
        step: number;
        collisionTween: Pixis.Tween;

        constructor(index: number, body: Box2D.Dynamics.b2Body, stage: Pixis.Stage, explosion: Pixis.Group, x: number, y: number) {
            body.SetUserData(index);

            var offset2 = { x: radius * scale / 4, y: 0 },
                rotation = Math.random();
